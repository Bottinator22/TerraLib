...
function renderutil.scanColours(p)
    if root.assetImage then
        ...
        local img = root.assetImage(p)
        local size = img:size()
        local colours = {}
        for x=1,size[1] do
            for y=1,size[2] do
                local pixel = img:get(x-1,y-1)
                if pixel[4] ~= 0 then
                    local exists = false
                    for k,v in next, colours do
                        if v[1] == pixel[1] and v[2] == pixel[2] and v[3] == pixel[3] and v[4] == pixel[4] then
                            exists = true
                            break
                        end
                    end
                    if not exists then
                        table.insert(colours,pixel)
                    end
                end
            end
        end
...
local threshold = 10
local thresholdInv = 360-threshold
local thresholdSat = 5
local function getV(c)
    return math.max(c[1],c[2],c[3])/255
end
function renderutil.scanPalettes(p)
    local colours = renderutil.scanColours(p)
    local palettes = {}
    for k,v in next, colours do
        local hsv = renderutil.toHSV(v)
        local makeNew = true
        for k,v2 in next, palettes do
            local diff = math.abs(v2.hue-hsv[1])
            if diff < threshold or diff > thresholdInv and (hsv[3] == 1 or math.abs(v2.saturation-hsv[2]) < thresholdSat) and hsv[2] ~= 1 then
                -- similar or identical hue
                makeNew = false
                table.insert(v2.colours, v)
                break
            end
        end
        if makeNew then
            local palette = {hue=hsv[1],saturation=hsv[2],colours={v}}
            table.insert(palettes,palette)
        end
    end
    -- sort colours in palettes by value, saturation if value = 1, brightest colours first
    for k,v in next, palettes do
        table.sort(v.colours, function(a,b)
            local hsva = renderutil.toHSV(a)
            local hsvb = renderutil.toHSV(b)
            if hsva[3] == 1 and hsvb[3] == 1 then
                return hsva[2] < hsvb[2]
            else
                return hsva[3] > hsvb[3]
            end
        end)
        v.darkestV = getV(v.colours[#v.colours])
        v.brightestV = getV(v.colours[1])
    end
    return palettes
end
...
function renderutil.toHexColour(c)
    ...
    for k,v in next, c do
        -- round and bound the inputted number
        r[k] = math.max(math.min(math.floor(c[k]),255),0)
    end
    if r[4] then
        return string.format("%02x%02x%02x%02x",table.unpack(r))
    else
        return string.format("%02x%02x%02x",table.unpack(r))
    end
end
